...
def sample_spans(shape, spans):
    ...
    for span in spans:
        ...
        for cinfo in span:
            ...
            denom = cinfo.mask.sum(axis=0).astype(np.float32)
            means = np.divide(totals,
                              denom,
...
def visualize_span_points(name, small, span_points, corners):
    ...
    for i, points in enumerate(span_points):
        ...
        mean, small_evec = principal_axis(points)
...
def main():
    ...
    for imgfile in sys.argv[1:]:
        ...
        if img is None:
            print('skipping', imgfile, 'because image could not be loaded')
            continue
